...
export class ApiService {
  ...
  login(email: string, password: string): Observable<{ user: unknown; token: string }> {
    return this.http.post<{ user: unknown; token: string }>(
      `${API}/auth/login`,
      { email, password }
    );
  }
  ...
  getExecutiveSummary(): Observable<ExecutiveSummaryData> {
    return this.http.get<ExecutiveSummaryData>(`${V1}/forecast/executive-summary`);
  }
