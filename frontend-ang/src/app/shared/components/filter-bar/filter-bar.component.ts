...
@Component({
  ...
  template: `
    <div class="bg-white border-b border-gray-200 px-6 py-3 flex items-center gap-3 flex-wrap">
      ...
      <div class="relative flex items-center" *ngFor="let f of filterDefs">
        <select
          [ngModel]="getDisplayValue(f.key)"
          (ngModelChange)="onUpdate(f.key, $event)"
          class="appearance-none text-xs text-gray-700 bg-white border border-gray-200 rounded-lg pl-2 pr-6 py-1.5 focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500 cursor-pointer min-w-[120px]">
      ...
      <button *ngIf="hasActiveFilters" (click)="filterService.clearFilters()"
