...
def identify_missing_skills(all_skills, canonical_groups):
    ...
    print(f"Found {len(missing_skills)} skills not covered by existing canonical groups")
...
def parse_llm_response_phase2(response_text, original_canonicals):
        ...
        for group_name, data in parsed.items():
            if isinstance(data, dict) and 'canonicals' in data:
                ...
                if isinstance(canonicals, list):
                    ...
                    print(f"Warning: 'canonicals' field is not a list for group '{group_name}': {canonicals}")
                    group_to_canonicals[group_name] = [str(canonicals)]
            elif isinstance(data, list):
                group_to_canonicals[group_name] = data
            else:
                print(f"Warning: Unexpected data format for group '{group_name}': {type(data)}")
    ...
    if missing_canonicals:
        print(f"Adding {len(missing_canonicals)} missing canonicals as individual groups")
        for canonical in missing_canonicals:
            group_to_canonicals[canonical] = [canonical]

    grouped_count = sum(1 for canonicals in group_to_canonicals.values() if isinstance(canonicals, list) and len(canonicals) > 1)
    total_groups = len(group_to_canonicals)
    print(f"LLM created {grouped_count}/{total_groups} actual merged groups (multi-canonical)")

    return group_to_canonicals


def sanitize_group_variants(group_mapping, allowed_skills):
    """Ensure every variant in each canonical group exactly matches allowed skills."""
    cleaned = {}
    for canonical, data in group_mapping.items():
...
def call_llm_for_higher_level_grouping(canonical_batch):
    """Call LLM to perform higher-level grouping of canonical names."""
...
Be aggressive about creating umbrella categories for related technologies!
...
### CONSTRAINTS
- Only merge CLEARLY related technologies
- Keep unrelated technologies separate: "Java" ≠ "JavaScript"
- Prefer broad, meaningful category names
- Only use canonical names exactly as provided; do not create, alter, or expand canonical strings
- Do not invent any new skills, canonical names, or placeholders beyond the provided list
...
def merge_canonical_groups(group_to_canonicals, original_canonical_groups, single_variant_groups):
...
def main():
    ...
    input_file_path = 'skills/skill_normalization_llm.json'
    output_file_path = 'skills/skill_normalization_llm_final.json'
    ...
    print(f"Total canonical groups after adding missing skills: {len(all_canonical_groups)}")
    total_variants_before = sum(len(group['variants']) for group in all_canonical_groups.values())
    ...
    print(f"Processing {len(canonical_names)} canonical names for higher-level grouping")
    ...
    for letter in sorted(letter_groups.keys(), key=lambda x: (x != '.', x)):
        ...
        batch_size = 100  # Smaller batches for higher-level grouping
        if len(canonical_batch) > batch_size:
            print(f"Large batch detected ({len(canonical_batch)} canonicals), splitting into chunks")

            letter_results = {}
            for i in range(0, len(canonical_batch), batch_size):
                chunk = canonical_batch[i:i + batch_size]
                print(f"Processing chunk {i//batch_size + 1}/{(len(canonical_batch) + batch_size - 1)//batch_size} ({len(chunk)} canonicals)")
        ...
        for group_name, canonicals in letter_results.items():
            if group_name in all_merged_groups:
                ...
                print(f"Merged canonicals for '{group_name}': {len(existing_canonicals)} + {len(canonicals)} → {len(merged_canonicals)} total")
            else:
                all_merged_groups[group_name] = {'canonicals': canonicals}

        total_processed += len(letter_results)
        print(f"Letter {letter} completed. Current merged groups: {len(all_merged_groups)}")
    ...
    final_groups = merge_canonical_groups(all_merged_groups, canonical_groups, single_variant_groups)

    # If merging failed (empty variants), fall back to original canonical groups
    if final_groups and all(len(group['variants']) == 0 for group in final_groups.values()):
        print("Merging failed, falling back to original canonical groups...")
        final_groups = all_canonical_groups

    # Remove any invalid variants and sort final groups alphabetically
    final_groups = sanitize_group_variants(final_groups, all_skills)
    ...
    if total_variants != total_unique_skills:
        print(f"CRITICAL: Variant count mismatch! Expected {total_unique_skills}, got {total_variants}")
        ...
        if missing_from_final:
            print(f"Adding {len(missing_from_final)} missing skills as individual canonical groups")
    ...
    total_final_variants = sum(len(group['variants']) for group in final_groups.values())
    ...
    if total_final_variants == 3490:
        ...
        print(f"CRITICAL ERROR: Expected {3490} skills, got {total_final_variants} ({3490 - total_final_variants} still missing)")

    # Ensure we have all 3490 skills
    if total_final_variants == 3490:
        print("SUCCESS: All 3490 skills accounted for!")
    else:
        print(f"WARNING: Expected {3490} skills, got {total_final_variants} ({3490 - total_final_variants} missing)")
